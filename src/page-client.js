...
function clientApp() {
  ...
  function bucketMixMarkup(analysis) {
    ...
    const slices = segments.map(({ index, bucket, count, start, end, span }) => {
      ...
      const path = pieSlicePath(cx, cy, outerRadius, innerRadius, start + gap / 2, end - gap / 2);
    ...
    const legend = segments.map(({ index, bucket, count }) => {
      const percent = total > 0 ? Math.round(count * 100 / total) : 0;
      const shotLabel = count + ' ' + (count === 1 ? 'shot' : 'shots');
      return '<button class="donut-legend-row" type="button" data-donut-key="' + bucket.key + '" style="--legend-color:' + bucket.color + ';--legend-delay:' + (index * 55) + 'ms" aria-label="' + escapeHtml(bucket.label) + ': ' + percent + '%, ' + shotLabel + '"><span class="donut-legend-label"><i></i>' + escapeHtml(bucket.label) + '</span><span class="donut-legend-value"><strong>' + percent + '%</strong><small>' + shotLabel + '</small></span></button>';
    }).join("");
    return '<div class="donut-shell"><span class="donut-orbit donut-orbit-one" aria-hidden="true"></span><span class="donut-orbit donut-orbit-two" aria-hidden="true"></span><svg class="donut-chart" viewBox="0 0 264 244" role="img" aria-label="Shot distribution by extraction time">' + donutDefsMarkup() +
  ...
  function donutDefsMarkup() {
    return '<defs><radialGradient id="donut-aura" cx="50%" cy="45%" r="65%"><stop offset="0" stop-color="#c99b64" stop-opacity=".22"></stop><stop offset=".52" stop-color="#8b6b4e" stop-opacity=".06"></stop><stop offset="1" stop-color="#171513" stop-opacity="0"></stop></radialGradient><radialGradient id="donut-core" cx="42%" cy="34%" r="76%"><stop offset="0" stop-color="#302a25"></stop><stop offset=".72" stop-color="#171513"></stop><stop offset="1" stop-color="#0e0d0c"></stop></radialGradient><linearGradient id="donut-slice-under20" x1=".1" y1="0" x2=".92" y2="1"><stop offset="0" stop-color="#c6d6e8"></stop><stop offset=".54" stop-color="#899eb7"></stop><stop offset="1" stop-color="#5d7692"></stop></linearGradient><linearGradient id="donut-slice-20to25" x1=".1" y1="0" x2=".92" y2="1"><stop offset="0" stop-color="#e5d3b6"></stop><stop offset=".54" stop-color="#b49f82"></stop><stop offset="1" stop-color="#84745e"></stop></linearGradient><linearGradient id="donut-slice-25to28" x1=".1" y1="0" x2=".92" y2="1"><stop offset="0" stop-color="#c0dec5"></stop><stop offset=".54" stop-color="#92b79c"></stop><stop offset="1" stop-color="#668d72"></stop></linearGradient><linearGradient id="donut-slice-28to30" x1=".1" y1="0" x2=".92" y2="1"><stop offset="0" stop-color="#ecc998"></stop><stop offset=".54" stop-color="#c99b64"></stop><stop offset="1" stop-color="#9c7044"></stop></linearGradient><linearGradient id="donut-slice-over30" x1=".1" y1="0" x2=".92" y2="1"><stop offset="0" stop-color="#ebbbb2"></stop><stop offset=".54" stop-color="#d08c7d"></stop><stop offset="1" stop-color="#9e5e55"></stop></linearGradient><filter id="donut-depth" x="-30%" y="-30%" width="160%" height="170%"><feDropShadow dx="0" dy="10" stdDeviation="10" flood-color="#000" flood-opacity=".3"></feDropShadow><feDropShadow dx="0" dy="-1" stdDeviation="1.5" flood-color="#fff" flood-opacity=".12"></feDropShadow></filter></defs>';
  }
